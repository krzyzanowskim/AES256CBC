...
final public class AES256CBC {
    ...
    public class func encryptString(str: String, password: String) -> String? {
        if str.characters.count > 0 && password.characters.count == 32 {
...
final private class AESCipher {
    ...
    func decrypt(bytes:[UInt8]) throws -> [UInt8] {
        ...
        return try PKCS7().remove(blockMode.decryptBlocks(blocks, iv: self.iv, cipherOperation: decryptBlock), blockSize: AESCipher.blockSize)
    }
...
private struct CBCBlockMode {
    ...
    private func encryptBlocks(blocks:[[UInt8]], iv:[UInt8]?, cipherOperation:CipherOperationOnBlock) throws -> [UInt8] {
    ...
    private func decryptBlocks(blocks:[[UInt8]], iv:[UInt8]?, cipherOperation:CipherOperationOnBlock) throws -> [UInt8] {
        precondition(blocks.count > 0)
        guard let iv = iv else {
            throw BlockError.MissingInitializationVector
        }
